...
func NewLogParser() *LogParser {
	return &LogParser{
		...
		apacheRegex: regexp.MustCompile(`(?P<ip>\S+) - - \[(?P<date>\d+/\w+/\d+):(?P<time>\d+:\d+:\d+) (?P<timezone>[\+\-]\d+)\] "(?P<method>\S+) (?P<path>\S+) (?P<http_version>[^"]+)" (?P<status>\d+) (?P<size>\d+|-) "(?P<referrer>[^"]*)" "(?P<user_agent>[^"]*)"`),
...
func (p *LogParser) parseApacheLog(logline string) ParsedLog {
	...
	fmt.Println("subexp names:", p.apacheRegex.SubexpNames())
